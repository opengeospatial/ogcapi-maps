...
A map resource can be obtained from a non map resource by adding `/map` the resource. The core does not specicy the resources where `/map` can be applied. However, we can list common places in and API where maps can be obtained in following table. Additional conformance classes will better define this posisbilities.
...
^e^     Possible link `rel` values are: `http://www.opengis.net/def/rel/OGC/1.0/style` for a URL pointing to other styles of the resource exposed as maps, 'maps' for a URL template to the map resource. 'alternate' for a URL pointing to another representation of the MapMetadata (e.g a TileJSON file).
|===
...
If the API has a OpenAPI document describing the schema fro the response, the following JSON Schema can be used to defined the response.
