...
[source,JSON]
  "styles": [
  {
    ...
    "links": [
      ...
      {
        "href": "https://example.com/api/1.0/styles/night/legend?f=png",
        "type": "image/png",
        "rel": "icon"
        "title": "Image representing the legend of the map"
      },
      {
        "href": "https://example.com/api/1.0/styles/night/thumbnail?f=png",
        "type": "image/png",
        "rel": "preview"
...
=================
Only the property `id` is mandatory designates the id of the style. The word `default` is reserved to designate the default style. However, the default style does not need to be called `default` (see the following recommendation about defaultStyle)
...
Depending on the subseting approach selected, other link objects and properties will be added by extensions to this core.
