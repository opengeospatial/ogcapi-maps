...
When only one of the two dimension parameters is provided and no specific bounding box is requested, the value of the other could be be used to return a square map by default.
...
However, in general circumstances (e.g., 2D representations on a screen), the client should make an effort to provide parameters that preserve the aspect ratio.
