...
This document describes two main ways to subset maps. Subsetting can be used to implement zoom and pan in a web map browser. The combination of <<rc_scaling>> and <<rc_spatial partitioning>> conformance classes provides a way to subset by an arbitrary bounding box (in a CRS). However, the approach will not benefit from cache mechanisms. Instead, a map can be retrieved as tiles in a predefined TileMatrixSet. This extension describes how to use OGC API maps in combination with OGC API tiles to by making maps accessible as tiles.

NOTE: The concept of map tiles substitutes the WMTS 1.0 usage of tiles. WMTS was focused on map tiles (tiles that are rendered using an style) while OGC API Tiles provides support to both data tiles and map tiles.
