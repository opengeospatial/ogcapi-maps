...
=== Styled resources
Geospatial resources can be modified or complemented by styles creating new endpoints giving access to the modified or complemented resources. The way this is done to a dataset resource or to a collection is specified in the draft _OGC API – Styles_ Standard.
...
=== Styled Resource Map
Styled resources are defined by the draft OGC API – Styles Standard. Nevertheless, it does not specify how to retrieve a map resource representing a styled resource. This section explains how to do this.
