...
The Collection Map requirements class specifies how to get maps from particular resources that contain geospatial data. Common resources that can contain geospatial data are the ones at the endpoint `/collections/{collectionId}` defined by the _OGC API - Common_ or _OGC API - Features_ Standards.
...
NOTE: The concept of a geospatial resource path substitutes the concept of "layer" in WMS but it intends to give a better integration between data visualization and data access.
