...
=== Conformance Class "OpenAPI 3 Specification"

==== Requirement OpenAPI definition completeness
[cols=">20h,<80d",width="100%"]
|===
|Test id: |/conf/oas3/completeness
|Requirement: |/req/oas3/completeness
|Test purpose: | Verify that the implementation completely and correctly describes the map resources
|Test method: | *Given:* an API conforming to _OGC API - Common - Part 1: Core_ "OAS30" conformance class or any other OGC API - Common conformance class declaring support to other subversion of OpenAPI version 3 +
*When:* retrieving the OpenAPI 3 API description +
*Then:* (assert that all parts of the requirement are fully satisfied)
|===
...
|Test method: | *Given:* an API conforming to _OGC API - Common - Part 1: Core_ "OAS30" conformance class +
