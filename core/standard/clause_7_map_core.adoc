...
A map resource can be obtained from a non map resource by adding `/map` to the resource URI. The Maps API core does not specify to which resources `/map` is applicable. 
However, a list of common paths to resources in the API, where maps can commonly be obtained is presented in the following table. Additional requirements classes better define these possibilities.
