...
In WMS, the `capabilities` of the service as a whole and the individual `layers` sections provide some metadata about the server and the layers that inform the client on some layer characteristics and some restrictions useful to formulate a successful map query. In the OGC API maps, this metadata is mainly provided by the service-meta relation in the landing page and the characteristics of the collections obtained when a collection is requested. This standard does not impose any extra requirement on this matter. However, implementers of Web APIs are encouraged to make use of the mechanisms provided by other standards of the OGC API family to communicate the relevant metadata to the client. The following table provides a reference to where some of this metadata aspects are specified.

[#where-map-metadata-is,reftext='{table-caption} {counter:table-num}']
.Where some map metadata elements are specified
[width = "100%",options="header"]
|===
| Name in WMS 1.3 | Where in the API | property | Specified in
