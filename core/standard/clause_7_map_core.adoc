...
A map resource can be obtained from an arbitrary geospatial resource by adding `/map` to the resource URI. The core requirements class does not specify to which resources `/map` is applicable. 
...
.Example of a successful HTTP response body containing a JPEG image captured by LANDSAT in January 2020
image::images/landsatMiraMon.jpeg[width=600,align="center"]
