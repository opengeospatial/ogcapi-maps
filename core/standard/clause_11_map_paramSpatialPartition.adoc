...
The geospatial resource covers a geospatial extent. This does not preclude that the map resource presents its own extent that will be similar or smaller to the geospatial extent resource (the API may opt to expose only a part of the geospatial resource as a map subsets)
...
include::recommendations/bbox/PER_map-outside-bounds.adoc[]
