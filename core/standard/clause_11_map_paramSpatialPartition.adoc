...
This class defines the extension to the core to formulate a subset using two mechanisms, one similar to the WMS 1.3 BBOX and the other similar to the WCS 2.0 subset. The client will select between the `bbox` and the `subset` mechanism.
...
NOTE: The collections description will contain a native or storage CRS. If it is not there then we will assume CRS84. The exact mechanism to do so will be defined by OGC API common.

NOTE: Provide the extent of the collection in native native CRS (also called or storage CRS) is recommended and the mechanism will be defined by OGC API common.

NOTE: A list of supported CRS's will be provided by the the collection. The first should be storage CRS. The mechanism to do so will be defined by OGC API common.
