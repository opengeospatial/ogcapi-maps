...
NOTE: The attributes max-width and max-height are indented to limit the work load of the server, by providing limitations in size of the outputs of the subset. If your width and height parameters are defined in relation to the size of a common device screen, please consider that new devices are being build with more and more pixels and a reasonable limit last year may be too restrictive with devices emerging this year.

==== Operation

The core of Part 1 defines how to get a map. This standard specifies parameters needed to scale a map.

==== Parameters width and height
These parameters indicate the size of the viewport in rows and columns where the map is going to be shown. If the format of the response is a raster image, the number of columns and rows of the image will commonly match the requested width and height number respectively
