...
The Maps API Standard core class provides a way to retrieve the map that is modified by other classes allowing for subsetting and scaling the map. These classes (<<rc_scaling, Scaling>>, <<rc_display-resolution, Display resolution>> and <<rc_spatial-subsetting, Subsetting>>) define the following parameters:
...
The following tables present an overview of the different combinations possible depending on whether the _Scaling_, _Subsetting_ or both _Scaling_ and _Subsetting_ requirements classes are supported by the implementation.

NOTE: The parameter `mm-per-pixel` is not included in these tables, but is used to infer the `scale-denominator` from the dimensions (`width` and `height`) and the spatial extent (`bbox`). If not provided in the request, it defaults to 0.28 mm per pixel.
...
| _none_                                              | `width`, `height`, `scale-denominator` and `center` | `bbox`
!===
...
| `bbox`, `width`, `height` and `scale-denominator`             | _Error (conflicts with default or provided `mm-per-pixel`)_
...
| `scale-denominator` ^1^                               | `width`, `height` and `center`                      | `bbox`
| `scale-denominator` ^1^ and `center`                  | `width` and `height`                                | `bbox`
| `width`, `height` and `scale-denominator`           2+| _Error (would require rescaling the map)_
...
| `width`, `height`, `scale-denominator` and `center` 2+| _Error (would require rescaling the map)_
3+|
^1^ The `scale-denominator` parameter is defined in the _Scaling_ requirements class.
However, an implementation supporting only _Subsetting_ may (*but is not required to*) still recognize the `scale-denominator` parameter and adjust its default `width` and `height` dimensions accordingly,
resulting in a different computed subsetting bounding box. In this case, a Subsetting-only implementation may not be applying scale-dependent symbolization rules correctly, since it likely would not render the map anew,
...
// page break to table header and table together in PDF
<<<
...
| `width`, `height` and `scale-denominator`                     2+| _Error (would require subsetting the map)_
...
| Parameters provided in the request                  | Server or resource defaults used                    | Computed
| `width` and `height`                                | `scale-denominator` and `center`                    | `bbox`
| `bbox`                                              | `width` and `height`                                | `center` and `scale-denominator`
| `center`                                            | `width`, `height` and `scale-denominator`           | `bbox`
| `width`, `height` and `center`                      | `scale-denominator`                                 | `bbox`
| `scale-denominator`                                 | `width`, `height` and `center`                      | `bbox`
| `scale-denominator` and `center`                    | `width` and `height`                                | `bbox`
| `width`, `height` and `scale-denominator`           | `center`                                            | `bbox`
| `bbox`, `width` and `height`                        | _None (fully defined combination)_                  | `scale-denominator`
| `bbox` and `scale-denominator`                      | _None (fully defined combination)_                  | `width` and `height`
| `width`, `height`, `scale-denominator` and `center` | _None (fully defined combination)_                  | `bbox`
!===
