...
The modular and reusable components used in this example OpenAPI definition can be found https://schemas.opengis.net/ogcapi/maps/part1/1.0/openapi .
...
A map is a portrayal of data resulting from applying a style (usually 2D image formats such as PNG or JPEG format or in presentation formats such as SVG).
The way the styling rules in a style are applied to the data to create the portrayal is out of scope of this standard (see https://github.com/opengeospatial/ogcapi-styles[_OGC API - Styles_],
as well as specific styles and symbology standards such as https://github.com/opengeospatial/styles-and-symbology[OGC Styles & Symbology], for details).

==== Implementing _OGC API - Maps_ within a Web API
A map can be delivered as a single static resource (only implementing the "Core" requirement class), or as a dynamic service able to return different maps for arbitrary extents (implementing "Subsetting" requirement class).
