...
Although this standard is designed as a building block that can be leveraged by other standards by adding precision about specific types of data available as maps
...
the minimum and maximum cell size and equivalent scale denominators. The domainset can enhance the schema to include bounds and resolution for additional dimensions, which is able to handle
...
normative requirements describing how to retrieve specific map content, or allowing the description of greater detail the domain or the observed or
