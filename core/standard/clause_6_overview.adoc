...
fixedHeight ^7^ | N/A | |
4+| ^1^ In WMS it was possible to specify as many bounding boxes as needed, one for each CRS. In OGC API - Maps, it is only available for native CRS (storageCrs). 
...
^4^ The `cascaded` XML attribute in WMS is removed because no practical use has been seen in past. 'capacaded' indicated if a map was generated by the addressed service of by another service assisting the first one.
