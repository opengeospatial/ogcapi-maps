...
| `bbox`, `width` and `height`                        | _None (fully defined combination^1^)_               | `scale-denominator`
| `bbox` and `scale-denominator`                      | _None (fully defined combination^2^)_               | `width` and `height`
...
based on different considerations for preserving the aspect ratio of the map for CRSs that do not have a constant mapping to physical distance along their axes, such as geographic CRSs
(preserving the aspect ratio of CRS unit vs. preserving the aspect ratio of physical units).
For example, an implementation may decide to maintain a constant degrees of longitude to degrees of latitude mapping for CRS84, while another may try to avoid returning maps that appear
squished at polar latitudes by compensating for the fact that degrees of longitudes correspond to a smaller real-world physical distance.
This may result in the `bounding box`, `width` or `height` being computed differently between these implementations.
