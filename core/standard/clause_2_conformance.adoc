...
NOTE: The "Custom Projection CRS" requirements class is not included in this sub-section because its primary focus is not defining resources, but that requirements class also specifies a `/projectionsAndDatums` resource listing values available for the parameters it defines.
