...
*<<rc_tileSets-list>>* (http://www.opengis.net/spec/ogcapi-maps-1/1.0/req/tilesets-list)
...
The _display resolution_ conformance class allows to specify the resolution of the display (the size of a pixel in millimeters) using the
...
.Example requests using center point
```
.../map?center=-120,60
.../map?center-crs=[EPSG:3857]&center=750000,70000
```
...
or the `subset` parameter for the `datetime` dimension.

.Example requests
```
.../map?datetime=2018-02-12T23:20:52Z
.../map?subset=datetime('2018-02-12T23:20:52Z')
```
...
| <<rc_tileSets-list>>         | http://www.opengis.net/spec/ogcapi-maps-1/1.0/conf/tilesets-list
