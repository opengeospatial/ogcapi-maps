...
^|A |The implementation SHALL include in its response for the `/projections` resource the complete list of custom CRS projection operation methods supported for map retrieval operations.
^|B |The implementation SHALL include in its response for the `/projections` resource the complete list of custom CRS datums operation methods supported for map retrieval operations.
...
NOTE: Refer to the OpenAPI definition for the `/projections` path resource for the schema corresponding to these requirements.
