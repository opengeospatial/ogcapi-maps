...
part:: If geospatial resource has a temporal aspect (e.g., if a temporal extent is defined in the collection description or in the dataset landing page), the headers of the response SHALL include a "Content-Datetime" header with the actual datetime instant or datetime interval of the rendered map  expressed in RFC3339 format.
