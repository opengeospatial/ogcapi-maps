...
^|H |When the `height` parameter is omitted, the implementation SHALL use an appropriate height which accurately reflects the default or requested scale of the map vertically, specifically for the local subset (bounding box) of the map being returned.
2+|
