...
^|C |The implementation SHALL establish the correspondence between real-world units and pixel units based on the equation: _physicalMetersPerPixel_= (`mm-per-pixel` / 1000 mm/m) * `scale-denominator`, where the _physicalMetersPerPixel_ do not necessarily correspond to CRS units for the region of that CRS corresponding to the subset of the map being returned, even if those units are expressed in meters.
