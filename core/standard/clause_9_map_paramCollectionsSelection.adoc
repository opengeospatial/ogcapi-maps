...
This requirements class can be used e.g. in conjunction with the <<rc_datasetMaps>> requirements class. A requirements class equivalent to this one is included in _OGC API - Tiles_.

=== Operation

By default, the geospatial data resources to be included in the dataset maps responses are unspecified but should represent the dataset as a whole.
...
If the value of the parameter `collections` contains a resource id of URI that does not exist on the Web API or too many collections are requested, the status code of the response is 400.
