...
* the one indicated in the `crs` parameter (the one picked for the map response). This possibility is described in the <<rc_crs, "CRS" requirements class>>.
...
The dimensions that can be used within the value specified for this parameter, in the context of this particular requirements class, are the abbreviations for the spatial axes specified in the CRS definition.
...
NOTE: Although the `scale-denominator` parameter is defined in the "Scaling" requirements class and not in "Spatial Subsetting",
