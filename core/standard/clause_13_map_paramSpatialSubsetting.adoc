...
==== Parameter `bbox`
The `bbox` parameter defines how to subset a map using a bounding box (in bbox-crs coordinates). The `bbox` parameter values are defined using the exterior edges of the pixels of the viewport defined by the width and height values. In other words, the relation of the Bounding Box to the map pixel matrix is that the bounding box goes around the "outside" of the pixels of the map rather than through the centers of the map's border pixels (as is often the case for coverages where cell values represent a point). In this context, individual pixels represent a cell area on the ground.
