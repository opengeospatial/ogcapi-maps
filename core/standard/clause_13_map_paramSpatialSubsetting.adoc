...
This requirements class adds the ability to request an arbitrary 2D or 3D spatial subset of a map using either a bounding box specified using one of two equivalent syntaxes
...
The way the third (vertical) spatial dimension is applied to filter the output is left to the implementation and may depend on the nature of the data being represented and/or the negotiated output format.
...
In this last example, the subset mechanism would likely only be used to subset the vertical dimension, while another mechanism would be used to select an arbitrary path in the horizontal dimensions.

[[maps-spatial-subsetting-crs]]
=== CRS for subsetting
In WMS 1.3 the CRS parameters had two purposes: it indicated the CRS of the data rendered in the map and it was the CRS of the coordinates of the BBOX parameter.
In this Standard, these two objectives have been separated. This section defines the CRS parameters for subsetting.

The CRS advertised in the collection's spatial extent is always in http://www.opengis.net/def/crs/OGC/1.3/CRS84 (for Earthly data and as required by _OGC API - Features_). At least for _OGC API - Maps_ and _OGC API - Coverages_, a native CRS (also called storage CRS) is defined. The `Content-Crs:` header (see <<maps-core-overview>>) specifies the CRS used in the response to avoid confusion.
...
==== Parameter `bbox`
This parameter defines how to subset a map by bounding box (in `bbox-crs` coordinates). The `bbox` parameter values are defined considering the exterior edges of the pixels of the viewport defined by the width and height. In other words, the relation of the Bounding Box to the map pixel matrix is that the bounding box goes around the "outside" of the pixels of the map rather than through the centers of the map's border pixels (as is often the case for coverages where cell values represent a point). In this context, individual pixels represent a cell area on the ground.
...
NOTE: As defined in the core requirements class, the `Content-Bbox:` and the `Content-Crs:` response headers report the actual bounding box of the data returned.
