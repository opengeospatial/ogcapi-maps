...
While it is possible to support map tiles and map tilesets without conforming to this requirements class, implementing this requirements class means that the tiles resources support parameters
...
This requirements class describes how to provide access to a map as tilesets in combination with the https://docs.ogc.org/is/20-057/20-057.html[_OGC API - Tiles_] Standard, itself based on the https://docs.ogc.org/is/17-083r4/17-083r4.html[_OGC 2D Tile Matrix Set and Tileset Metadata_] Standard.
...
This requirements class specifies that the query parameters of the GET request to individual map tiles support as additional parameters
the ones defined in the _background_, _display resolution_, _spatial subsetting_ (if a vertical dimension is available), _general subsetting_, and _scaling_ requirements classes,
when any of those requirement class is supported.
...
class of the _OGC API - Tiles_ Standard. The response to map tiles requests shall also reflect the parameters as specified in operation requirement.
