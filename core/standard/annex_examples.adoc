...
As explained in the <<scaling-aspect-ratio, scale and aspect ratio considerations section>>, implementations may the compute dimensions and bounding boxes not explicitly specified slightly differently.
...
which completes the bounding box calculation, which expressed in the default CRS84 (longitude, latitude) order of the `bbox` parameter would be:
...
which completes the bounding box calculation, which expressed in EPSG:3395 together with the required `bbox-crs` would be:
