...
Before introducing the selection of an alternative output Coordinate Reference System (World Mercator, EPSG:3395) from the default native CRS (`storageCRS`) returned so far (CRS84),
let's zoom out significantly further away to a 1:20,000,000 scale. At the scales used above, the difference between those two CRSs would not be distinguishable,
