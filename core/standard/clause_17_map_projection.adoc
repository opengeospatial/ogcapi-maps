...
This requirements class defines the ability for clients to specify custom projection CRS based on parameters specifying a projection operation method, values for corresponding parameters, a datum, as well as an orientation.
...
Angles are specified in degrees, whereas identifiers for methods, parameters and datums can be specified as URIs or CURIEs.
...
In order to facilitate the task of centering a custom projection on the area of interest, which varies greatly between projections, the `crs-proj-center` parameter automatically sets the most appropriate parameters
