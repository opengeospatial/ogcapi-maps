[[rc_oas3]]
== Requirements Class "OpenAPI Specification 3" API definition

=== Overview

include::requirements/requirements_class_oas3.adoc[]

The OpenAPI requirements class defines the specifics of providing a definition of a Web API implementing the Maps API Standard using OpenAPI Specification version 3.
...
The (_oas30_) requirement class from _OGC API - Common - Part 1: Core_ provides many details on general requirements that this requirements class adopts by dependency. A later version of OGC API - Common may describe how to declaring support to other subversion of OpenAPI version 3. In addition, extra requirements are provided here. Also, the _OGC API - Common_ requirements class provides details on how to request an API definition.
...
include::requirements/oas3/REQ_completeness.adoc[]

===== Reusable API components

Reusable components for creating OpenAPI definitions for implementations of this OGC API can be found in https://schemas.opengis.net/ogcapi/maps/part1/1.0/openapi .
...
include::requirements/oas3/REQ_operation-id.adoc[]
