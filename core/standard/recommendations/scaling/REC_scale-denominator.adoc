...
part:: The map description document may include a minimum and maximum scale denominator indicating which scale range is accepted by the server. Out of this range the server will not be able to create a map. These characteristics are defined as (shown as OpenAPI Specification 3.0 fragment)
+
[source,YAML]
----
  minScaleDenominador:
