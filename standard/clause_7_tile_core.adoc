...
This standards assumes that data is organized into one or more collections. `Collections` provides information about and access to the collections.
...
A collection that can be retrieved as tiles will add specific object in the collection information about that will contain specific information necessary to get tiles in the resource collection response. In the core, the _tiles_ section only informs about the formats the tiles can be retrieved.

include::requirements/tiles/core/REQ_sfc-formats.adoc[]
...
In the absence of any other information, the client and service will assume support for Web Mercator Quad TileMatrixSet (http://www.opengis.net/def/tilematrixset/OGC/1.0/WebMercatorQuad) as defined in the Annex D.1 of the OGC 17-083r2 standard with no restriction in zoom level or area.
