...
Maps provide a visual representation of relationships between things within a defined space. This specification defines a standardized way to request images of maps over the Internet to serve multiple purposes (e.g. displaying maps in web pages, mapping software, etc.). The [Open Geospatial Consortium](https://www.ogc.org/) (OGC) approves and maintains the specification for public use. The draft specification is available in [HTML](http://docs.ogc.org/DRAFTS/20-058.html) and [PDF format](http://docs.ogc.org/DRAFTS/20-058.pdf). 
