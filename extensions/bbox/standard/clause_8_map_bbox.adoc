...
=== Map resource
The OGC API - Maps - Part 1: Core defines the map resource and the map description document. The core contains a minimum set of metadata to enable a client application to formulate a map request.

This class defines the extension to the core map description document needed to formulate a bounding box subset.

==== Map description document
include::requirements/core/REQ_link-relation.adoc[]
  ...
  "styles": [
  {
    ...
    "links": [
      ...
      ...
      {
        ...
        "title": "Access to a nice maps subsets"
      }
    ]
  }
]
...
include::recommendations/core/REC_link-relation.adoc[]
...
include::recommendations/bbox/REC_width-definition.adoc[]

include::recommendations/bbox/REC_height-definition.adoc[]

include::recommendations/bbox/REC_no-width-height-definition.adoc[]

==== Parameters transparency and bgcolor
These parameters indicate how the absence of data will be represented. The parameter `transparent` is deprecated and the use of the 'alpha channel' in `bgcolor` is recommended instead.d

include::requirements/bbox/REQ_transparent-definition.adoc[]

include::requirements/bbox/REQ_bgcolor-definition.adoc[]

==== Response

A successful response to a map bbox GET operation will be consistent with the media type of resource requested. This draft specification does not impose any media type or file format and map responses may be in JPEG, PNG or other appropriate format (including vector based formats such as KML or SVG).
